...
import { RelationshipType, Company, PaginatedResponse } from '../types';
...
const RelationshipMasterPage: React.FC = () => {
    ...
    const [response, setResponse] = useState<PaginatedResponse<RelationshipType> | null>(null);
    ...
    const loadData = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            const data = await api.fetchRelationshipTypes(companyData.comp_id, { page, search: searchQuery });
            setResponse(data);
        } catch (error) {
    ...
    const handleSave = async () => {
        ...
        try {
            await api.saveRelationshipType({ ...editingItem, comp_id: companyData.comp_id });
    ...
    const handleToggle = async (item: RelationshipType) => {
        const updated = { ...item, status: item.status === 1 ? 0 : 1 };
        try {
            await api.saveRelationshipType(updated);
